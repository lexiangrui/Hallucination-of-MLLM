...
\begin{frame}{POPE: Three Splits Detect Three Error Mechanisms}
  \begin{columns}[c,onlytextwidth]
    \column{0.56\textwidth}
      ...
      \begin{block}{2. Popular: global frequency bias}
        \scriptsize
        Absent objects are the most frequent COCO categories
        regardless of scene context (e.g., backpack is a
        globally common category).
        HR jumps to 2.40\%--6.20\% because the LLM prior $P_L$
        elevates high-frequency words when visual evidence is weak.
      \end{block}
      \vspace{-0.7em}

      \begin{block}{3. Adversarial: co-occurrence interference}
        \scriptsize
        Distractors are selected by co-occurrence statistics
        (e.g., bear co-occurs with person and book in training data).
        HR peaks at 4.80\%--8.90\% where $P_L$'s co-occurrence
        prior overrides $P_V$'s visual evidence.
      \end{block}
    ...
    \column{0.41\textwidth}
      ...
      fails to provide enough discrimination to counteract
      the co-occurrence prior.

      \vspace{0.2em}
      \textbf{Linguistic modality prior $P_L$.}
      The LLM backbone carries co-occurrence regularities from
      text-only pretraining. Popular isolates the unconditional
      frequency prior: models tend to affirm globally frequent
      words when visual evidence is weak. Adversarial activates
      the conditional co-occurrence prior, where objects that
      frequently appear together in training data trigger
      false positives.
  \end{columns}
\end{frame}

\begin{frame}{POPE Case Analysis}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \centering
      ...
      \vspace{0.25em}
      \footnotesize
      \textbf{Case 1: Popular split (global frequency bias)}\\
      ...
      \begin{exampleblock}{Case 1: global frequency bias}
        \footnotesize
        ``Backpack'' is one of the most frequent categories in COCO,
        selected as a negative regardless of scene context.
        When visual evidence is ambiguous, the model defaults to
        affirming high-frequency words learned during pretraining.
      \end{exampleblock}
    ...
    \column{0.48\textwidth}
      ...
      \vspace{0.25em}
      \footnotesize
      \textbf{Case 2: Adversarial split (co-occurrence interference)}\\
      ...
      \begin{exampleblock}{Case 2: co-occurrence interference}
        \footnotesize
        ``Bear'' co-occurs frequently with person and book
        in the COCO training statistics.
        $P_L$'s co-occurrence prior triggers a Yes answer,
        while $P_V$ fails to distinguish teddy bear from bear.
      \end{exampleblock}
  \end{columns}
\end{frame}
...
\begin{frame}{MathVista: Results and Hallucination Distribution}
  \begin{columns}[T,onlytextwidth]
    \column{0.5\textwidth}
      ...
      \vspace{0.5em}
      \textbf{\footnotesize Key observations: CoT effects}\\[0.1em]
      \scriptsize
...
\begin{frame}{MathVista: CoT Impact and Four Mechanisms}
  \begin{columns}[T,onlytextwidth]
    \column{0.45\textwidth}
      ...
      \vspace{0.2em}
      \begin{block}{Recent findings from prior work}
        \scriptsize
        This pattern is consistent with two recent studies on CoT
        in multimodal reasoning.
        \textbf{Liu et al. (2025)} show through attention analysis
        that as reasoning chains become longer,
        attention gradually drifts away from the image toward
        linguistic priors, producing
        ``more thinking, less seeing.''
        \vspace{0.25em}

        \textbf{Kancheti et al. (2025)} further report,
        across 17 evaluated models,
        that CoT consistently degrades visual-spatial reasoning
        because models begin to hallucinate visual details from
        textual priors.
        \vspace{0.25em}

        The sharp rise in Qwen faithfulness hallucinations under CoT
...
\begin{frame}{MathVista Case Analysis}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \centering
...
\begin{frame}{Error Analysis}
  \begin{columns}[c,onlytextwidth]
    \column{0.48\textwidth}
      \textbf{\small 1. Human Alignment Verification (Human-as-Judge) }
      \vspace{0.15em}
...
\begin{frame}{Error Analysis}
  \begin{columns}[T,onlytextwidth]
    \column{0.50\textwidth}
      \textbf{\small 2. Judge model consistency}
      ...
      \begin{tabular}{lccccc}
        ...
        Gemini 2.5-flash & 37.0\% & 27.0\% & \alert{$-$10.0} & 0.591 & 0.658 \\
        Qwen3.5-35b & 14.0\% & 18.0\% & $+$4.0 & 0.777 & 0.722 \\
        Qwen3-VL-235b & 15.0\% & 22.0\% & $+$7.0 & 0.638 & 0.669 \\
        \midrule
      ...
      \begin{itemize}\setlength{\itemsep}{0.18em}\setlength{\parskip}{0pt}
        \item \textbf{Substantial overall agreement}
              ...
              Qwen3.5-35b $>$ Qwen3-VL-235b $>$ GPT-5.4-mini $>$ Gemini 2.5-flash.
    ...
    \column{0.47\textwidth}
      \textbf{\small 3. Threshold sensitivity}
...
\begin{frame}{Error Analysis}
  \begin{columns}[T,onlytextwidth]
    \column{0.50\textwidth}
      \textbf{\small 4. Length bias analysis}
    ...
    \column{0.47\textwidth}
      \textbf{\small 5. Cross-model hallucination consistency}
...
\begin{frame}{Future Work}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.48\textwidth}
      \textbf{\footnotesize }{Inspiration for hallucination inhibition}
      \scriptsize
      \begin{itemize}\setlength{\itemsep}{0.1em}\setlength{\parskip}{0pt}
