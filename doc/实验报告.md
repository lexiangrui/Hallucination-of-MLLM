...
多模态大模型（MLLM）在视觉问答和图文推理中展现出了强大的能力，但它们依然容易产生幻觉。这些幻觉表现为：输出内容与视觉输入不一致、与世界知识相矛盾，或推理过程包含逻辑缺陷。本报告提出了一套框架，用于自动检测并评估 MLLM 中的幻觉现象。我采用了三类别分类体系（忠实性、事实性、逻辑性幻觉），在三个基准测试（POPE、MathVista、VQA-RAD）上，使用基于规则的检测和 MLLM-Judge 协议，对四个模型（GPT-5.4-mini、Gemini 2.5 Flash、Qwen3.5-35B-A3B、Qwen3-VL-235B-A22B）进行了评估。主要发现如下。

(1) 语言先验会结构性地凌驾于视觉感知之上。POPE 三个划分的实验隔离了两种模态先验的混淆效应：视觉编码器的参数化偏差（视觉先验 P_V）与 LLM 文本预训练习得的统计共现规律（语言先验 P_L）；二者通过独立的注意力路径影响输出。视觉信号模糊时 P_L 压制残余视觉信号（Popular 幻觉率 2.40%–6.20%），P_V 与 P_L 联合失效则产生最严重的幻觉率（Adversarial 4.80%–8.90%）。同一机制解释了 VQA-RAD 中忠实性幻觉占 96% 以上的现象：流利的医学知识无法弥补放射科视觉定位能力的不足。
(2) 思维链（CoT）提示词的效果取决于模型的基线表现：CoT 能显著减少较弱模型的幻觉（GPT-5.4-mini 降低 10.7 个百分点，Gemini 2.5 Flash 降低 6.7 个百分点），但会增加较强模型的幻觉率（Qwen3-VL 增加 7.7 个百分点）。我将此归因于四种机制的共同作用：结构化分解与视觉锚定有助于弱模型修正推理和感知错误；暴露效应与雪球效应则会让强模型将隐含的视觉误读暴露于推理链，并沿错误前提继续推演。

最后，五项独立的误差分析（人类对齐验证 Cohen's κ = 0.70；GPT-5.5 与 Claude Opus 4.7 的跨 Judge 一致性 κ = 0.78；阈值敏感性分析；长度偏差评估；以及跨模型重叠度分析）验证了该评估框架的可靠性。代码和数据见 https://github.com/lexiangrui/Hallucination-of-MLLM 。
...
多模态大模型（MLLM）在视觉问答和图文推理中展现出了强大的能力，但依然容易产生幻觉。具体表现为：输出内容与视觉输入不一致、与世界知识相矛盾，或推理过程包含逻辑缺陷。本报告重点研究 MLLM 中幻觉的检测与评估。评估要求总结如下：
...
本实验评估了四个 MLLM，涵盖闭源与开源模型，包含密集型与混合专家（MoE）架构（3B 至 235B 激活参数）。此外，还有另外两个模型在检测流程和跨 Judge 一致性检验（详见附录）中担任 MLLM Judge。表 1.1 总结了模型配置。
...
幻觉是 MLLM 已知的局限性。不同研究采用了不同的术语，但基本概念存在大量重叠。本节回顾三篇近期的综述，随后介绍本研究所采用的分类体系。

Bai 等人（2025）将 MLLM 的幻觉定义为”生成的文本与相应的视觉内容不一致”。他们将这一概念追溯到大语言模型（LLM），区分了事实性幻觉（偏离真实世界事实）和忠实性幻觉（偏离输入上下文或缺乏自洽性）。他们以物体为中心的分类体系包含三个层次：类别（捏造或错误识别物体）、属性（类别正确，但颜色、形状或数量等属性错误）以及关系（物体和属性正确，但空间或交互关系错误）。

Chen 等人（2026）提出了一个更广泛的统一定义，将幻觉视为生成内容与输入条件或既定世界知识之间的任何不一致。他们的分类扩展了“忠实性-事实性”二分法：忠实性幻觉根据视觉粒度分为物体级、属性级和场景级错误；事实性幻觉则分为常识错误和领域特定错误。
...
考虑到本报告的实验针对视觉问答和图文推理，我基于上述框架采用了以下定义及三类别的分类体系：

> **MLLM 的幻觉是指模型的回答与输入的视觉内容不一致、与可验证的现实世界知识相矛盾，或推理过程包含逻辑缺陷。**
...
**POPE 中的物体幻觉。** Li 等人（2023）将物体幻觉定义为视觉语言模型生成的描述或回答中，包含了与目标图像不一致或在图像中根本不存在的物体。POPE 将这一概念具体化为一项二元探测任务：针对每张图像，基准测试会提出类似“图像中是否存在 X？”的“是/否”问题。如果真实标签为“否”，而模型回答了“是”，则认为模型产生了一次物体幻觉。在二元分类的视角下，这是一个假阳性（False Positive）事件，对应物体级忠实性幻觉类别。
...
**提示词构建。** 在回答生成阶段，系统会根据样本的元数据动态构建提示词，以生成符合特定格式的指令。在直接回答（Direct）模式下，提示词由问题文本和格式指令拼接而成。在思维链（CoT）模式下，提示词进一步要求模型逐步推理，并用“Final answer:”标记最终答案。我评估了这两种模式，以分析显式推理对幻觉率的影响。
...
四个模型表现出截然不同的决策风格，其特征为幻觉控制能力（HR）和回答偏好（Yes Ratio）。图 4.1 展示了所有模型在三个划分上的幻觉率，可见模型间存在明显的性能分层。

**Qwen3-VL-235B** 在所有划分上都取得了最低的幻觉率（0.30%、2.40%、4.80%），且 Yes Ratio 始终稳定在 0.47 以下，暗示了强视觉编码器与均衡决策风格的搭配。

**Gemini 2.5 Flash** 的精确率与召回率保持均衡，并且是唯一一个幻觉率从 Popular（5.70%）到 Adversarial（5.10%）反而*下降*的模型，说明它应对共现干扰项（Adversarial）的能力甚至优于应对全局高频偏置（Popular）。

**Qwen3.5-35B-A3B** 在每个划分上都拿到了最高的召回率（≥0.87），但代价是全组最高的 Yes Ratio（在 Adversarial 上达到 0.529）和最差的 Adversarial 幻觉率（8.90%），符合宽松型决策风格的特征。
...
**难度逐级递增。** 从 Random（随机）到 Popular（常见）再到 Adversarial（对抗），大部分模型幻觉率都呈现出单调递增趋势（见表 4.1 和图 4.1）。这与 POPE 三层负样本采样设计一致。然而，除了难度排名之外，这三个划分还隔离出了三种在性质上截然不同的错误机制，对应本文的忠实性幻觉分类。
...
**Popular 划分：全局高频类别偏置。** 这里的负样本来自 COCO数据集中全局出现频率最高的 top-K 类别，它们在任何场景中都极为常见，但在当前特定图像中并不存在。此时，幻觉率急剧上升至 2.40%-6.20%，大约是Random 划分的5到8倍。这种剧增孤立地展现了高频语言先验的影响：当视觉证据模糊时，模型倾向于对频繁出现类别给出肯定回答，退回到预训练阶段学习到的统计分布规律。4.3.1节中的案例 1提供了一个直观的例子。

**Adversarial 划分：高频共现类别干扰。** 这里的负样本根据 COCO训练集共现统计选取，与图中已有物体频繁共现，但在当前特定图像中并不存在。幻觉率进一步上升至 4.80%-8.90%，此时模型被训练数据中的共现统计规律所干扰：语言模块中习得的条件共现概率凌驾于视觉感知之上。正如 4.3.2 节中的案例 2所示，这是三种失败模式中最困难的一种，其根本原因在于共现先验的强度超过了视觉信号的强度，这无法单靠增加模型规模来解决。
...
**对 POPE 三种错误机制的统一解释。** 回到本实验结果（4.2.2 节），三种错误模式恰好对应两类先验在不同条件下的此消彼长：Random 划分中负样本与场景无关，P_L 几乎不参与，0.30%–1.10% 的残存幻觉率来自 P_V 的基线偏差，反映视觉编码器感知能力的上限。Popular 划分是 P_L 高频先验最纯粹的表现—backpack 作为 COCO全局高频类别被选为负样本，其选取与图像场景无关。视觉证据模糊时，模型倾向于对这些高频词给出肯定回答，如案例1中四个模型全部错误断言图像中有背包，幻觉率攀升至2.40%-6.20%。Adversarial 划分则是 P_V 与 P_L 的联合失效：P_L 的共现先验让 bear 因与 person、book 高频共现而成为干扰项，P_V 则未能提供足够细粒度的视觉辨别来抵消这一共现偏置。两者叠加导致如案例2所示，四个模型均在语义上将其与 teddy bear 合并，产生了最严重的幻觉率（4.80%-8.90%）。这也解释了为何增大模型规模无法解决 Adversarial 幻觉（Qwen3-VL-235B 仍有 4.80%）—规模扩大主要强化 P_L 的统计精度，而共现先验恰恰随训练数据规模增强，并不因模型变大而减弱。

### 4.3 失败案例分析

#### 4.3.1 案例 1：全局高频类别偏置（Popular）

这个案例说明了 Popular 划分旨在隔离的全局高频类别偏置机制。图像展示了一个人在户外吃三明治和热狗的场景。问题询问图像中是否有背包，真实答案为”否”。所有四个模型都回答了”是”。
...
![POPE 案例 1：户外饮食场景。](../report/figures/pope_000000569839.jpg)

**根本原因：全局高频类别偏置。** “背包”是 COCO 数据集中频繁出现的类别之一，因此被 Popular 采样策略选为负样本，其选取与图像场景无关。语言模块在预训练中习得了频繁出现类别的高先验概率，当视觉信号模糊时，这一高频先验凌驾于视觉感知之上，导致模型给出肯定回答。Popular 划分利用的正是这一点：模型将训练数据中频繁出现类别的统计分布误用作视觉判断的依据。

#### 4.3.2 案例 2：高频共现干扰（Adversarial）

这个案例说明了在 4.2.2 节中指出的高频共现干扰机制。下表和对应图像展示了来自 Adversarial 划分的一个样本。图像中包含一只泰迪熊、一个人和一本书。问题询问图像中是否有一只熊，真实答案为”否”。所有四个模型都回答了”是”，这表明这是一个系统性的错误，而不是个别模型的缺陷。
...
![POPE 案例 2：沙发上的泰迪熊。](../report/figures/pope_000000567886.jpg)

**根本原因：高频共现干扰。** 根据 COCO 训练集的共现统计，”熊”与图像中已有物体（”人””书”）频繁共现，因此被 Adversarial 采样策略选为负样本。模型的语言模块习得了这一共现模式，条件共现概率构成了强烈的肯定偏置；同时，视觉编码器对 teddy bear 与 bear 的细粒度边界缺乏足够的辨别力，未能通过视觉信号抵消这一偏置。两者叠加，导致四个模型全部给出肯定回答。Adversarial 划分利用的则是另一种动态：P_L 的共现先验与 P_V 的感知局限联合失效，产生了比 Random 和 Popular 更高的幻觉率。
...
**表 5.1：MathVista Direct 与 CoT 模式综合结果。Δ 值 = CoT − Direct，负数表示 CoT 模式下幻觉减少，净改善为正表示整体表现变好。最佳值以粗体高亮。**

**（a）核心指标**

| 模型 | Direct HR | CoT HR | ΔHR | Direct 均分 | CoT 均分 | Δ均分 |
|---|---|---|---|---|---|---|
...
CoT 在 MathVista 上的影响按照 Direct 模式的基线表现划分。Direct 模式下幻觉率高的模型（GPT-5.4-mini 45.1%，Gemini 34.3%）在引入 CoT 后得到了显著改善；而 Direct 模式下幻觉率低的模型（Qwen3.5 13.3%，Qwen3-VL 16.3%）反而出现了退化，如下图所示。
...
我认为 CoT 主要通过四种机制同时减少和引入幻觉，前两者解释了较弱模型获得的提升，后两者解释了较强模型遭遇的退化：
...
*   **雪球效应 (Snowball effect)**：CoT 迫使模型在推理的早期做出明确的视觉或数学论断，一旦模型在推理早期产生幻觉，它就会围绕这个错误前提连贯地编造下去，导致错误沿着推理链不断传播和放大。Zhong 等人（2024）将这一现象形式化为"多模态幻觉雪球效应"（Multimodal Hallucination Snowballing），证明了早期幻觉会系统性地诱导后续步骤产生更多幻觉。

这四种机制共同解释了 CoT 的双向效应：当模型的基线推理能力较弱时，结构化分解和视觉锚定带来的收益超过暴露效应和雪球效应带来的损失；而当基线已经较强、单步答案错误率较低时，可改善的空间有限，但每一条额外推理步骤都引入了暴露效应和雪球效应的风险。

### 5.3 失败案例分析

#### 5.3.1 案例 1：CoT 雪球效应导致逻辑性幻觉（GPT-5.4-mini）
...
这一现象源于 4.2.3 节中的双模态先验分析。在放射科场景中，视觉编码器面对的是其训练分布中覆盖度较低的图像类型，通用视觉编码器主要从自然图像（COCO、ImageNet）中学习，缺乏对 X 光、CT 和 MRI 中解剖结构和病灶的细粒度辨别能力。与此同时，语言模块在医学文本预训练中积累了大量医学知识，模型能流利地讨论 DWI 序列、脑室周围白质和放射科视角，却无法可靠地判断病灶位于大脑的哪一半。当 视觉信号天生不足时，语言先验就会主导输出，产生视觉上错误但语言上流利的回答。
...
这本质上与 POPE Adversarial 划分（案例 2）中发现的机制相同：一旦出现了熟悉的上下文线索，模型就会退回到共现统计规律。在艾宾浩斯实验中，这种机制被推向了极端：“看到艾宾浩斯图 $\Rightarrow$ 回答一样大”这种被记住的关联，似乎完全覆盖了图像中真实存在的信号。
...
**概览。** POPE（Li et al., 2023）将物体幻觉评估转化为基于 MS COCO `val2014` 划分的二元物体存在性探测任务（Lin et al., 2014）。这一评估范式最早可追溯到 Rohrbach 等人（2018）在图像描述幻觉中提出的 CHAIR 指标，随后由 Li 等人（2023）形式化为统一的 polling 协议。
...
* **Popular**：从最常见的 top-K COCO 类别中采样图中不存在的物体，从而构造全局高频类别偏置。
* **Adversarial**：对每张图像，根据 COCO 训练集共现统计，选择与图中已出现物体共现频率最高的三个类别作为负样本，从而得到高频共现类别干扰。

这三个 split 构成了一个单调递增的难度梯度，并在 4.2.2 节中得到了实验观察。
