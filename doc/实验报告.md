...
(1) 语言先验会结构性地凌驾于视觉感知之上。POPE 三个划分的实验隔离了两种模态先验的混淆效应：视觉编码器的参数化偏差（视觉先验 P_V）与 LLM 文本预训练习得的统计共现规律（语言先验 P_L）；二者通过独立的注意力路径影响输出。视觉信号模糊时 P_L 压制残余视觉信号（Popular 幻觉率 2.40%–6.20%），P_V 与 P_L 联合失效则产生最严重的假阳性（Adversarial 4.80%–8.90%）。同一机制解释了 VQA-RAD 中忠实性幻觉占 96% 以上的现象：流利的医学知识无法弥补放射科视觉定位能力的不足。
...
**难度逐级递增。** 从 Random（随机）到 Popular（常见）再到 Adversarial（对抗），所有四个模型的幻觉率都呈现出严格的单调递增趋势（见表 4.1 和图 4.1）。这与 POPE 三层负样本采样设计一致。然而，除了难度排名之外，这三个划分还隔离出了三种在性质上截然不同的错误机制，对应本文的忠实性幻觉分类。
...
这个案例说明了在 4.2.2 节中指出的近义语义混淆机制。下表和对应图像展示了来自 Adversarial 划分的一个样本。图像中包含一只泰迪熊、一个人和一本书。问题询问图像中是否有一只熊，真实答案为“否”。所有四个模型都回答了“是”，这表明这是一个系统性的错误，而不是个别模型的缺陷。
...
这个案例说明了 Popular 划分旨在隔离的语言先验偏差机制。图像展示了一个人在户外吃三明治和热狗的场景。问题询问图像中是否有背包，真实答案为“否”。同样，所有四个模型都回答了“是”。
