...
比较 **MLLM Judge 自动化幻觉检测** 与 **人类标注者判断** 之间的一致性，验证自动检测方法是否可以作为人类判断的可靠代理。本实验借鉴 Omni-I2C (Zhou et al., 2026) 的人类对齐方法论框架。

### 1.1 核心研究问题

- **RQ1**：MLLM Judge 在 MathVista COT 场景下的幻觉判定与人类标注者的一致性有多高？
- **RQ2**：不同类型的幻觉（忠实性 / 事实性 / 逻辑性）在人类与自动检测之间的一致性是否存在差异？
- **RQ3**：MLLM Judge 的检测错误模式是什么？哪些类型的样例最容易出现人机分歧？
...
| **检测方法** | MLLM Judge（GPT-5.5, temperature=0.0） |
...
人类标注者与 MLLM Judge 均基于此三分类体系判断幻觉类型，确保后续类型一致性比较的合理性。
...
|------|------|-----------|------|
| 模型 | 4 | — | 全部四个评测模型 |
| MLLM Judge 判定 | 2 | 5 | 有幻觉(has_hallucination=1) / 无幻觉(0) |
...
**标注方式**：Single-blind（隐藏模型名称与 MLLM Judge 判定结果）
...
| **F1 Score** | 2PR/(P+R) | MLLM Judge 的幻觉检出能力 | — |
...
|----------|------|---------------|-------------------------------|
| **MLLM Judge 漏检** | 人类判定有幻觉，GPT 判定无 | 模型回答包含微妙的视觉不一致，MLLM Judge 未能捕获 | Under-detection |
| **MLLM Judge 过判** | 人类判定无幻觉，GPT 判定有 | MLLM Judge 将合理的推理步骤误判为逻辑错误 | Judge bias / Over-detection |
...
```
results/errors_analysis/human_alignment/
├── samples.csv            # 待标注样本集（空模板）
├── annotations.csv        # 标注完成后填写
├── meta.json              # MLLM Judge 判定结果（自动生成）
...
# Step 3: 人工标注 results/errors_analysis/human_alignment/annotations.csv
#         运行 python scripts/annotate.py 打开 http://localhost:8765
# Step 4: 生成对齐报告
